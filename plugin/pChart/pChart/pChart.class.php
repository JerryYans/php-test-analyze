...
class pChart
{
  ...
  /* Set the font properties */
   function setFontPropertiesDir($FontDir)
    {
       ...
       if ( $ID % $SkipLabels == 0 )
        {
         ...
         if ($d_count == $foreach_i){
         	$Value = $Value;
         }
         if ( $Angle == 0 )
          {
           $YPos = $this->GArea_Y2 + 28;//设置下面的字距离x轴的距离
         ...
         else
          {
           ...
           if ($d_count == $foreach_i && $this->FontDir && is_file("{$this->FontDir}/simfang.ttf")){
				$clr = ImageColorAllocate($this->Picture, 243, 247, 252);
				$fnt = "{$this->FontDir}/simfang.ttf"; // ttf字库的绝对路径
				//$str = iconv("GB2312","UTF-8","中国"); // 转换成utf-8编码，需加载iconv函数库。PHP ver >= 4.3.0
				$str = "周";
				ImageTTFText($this->Picture, 14, 0, floor($XPos)+15,$YPos, $clr, $fnt, $str);
           }
       ...
       foreach ( $Data as $Key => $Values )
        {
         ...
         if ( isset($Data[$Key][$Serie]) && is_numeric($Data[$Key][$Serie]))
         {
            ...
            $Positions = imagettfbbox($fontSize, 0,$this->FontName,$Value);
            $Width  = $Positions[2] - $Positions[6];
            $XOffset = $XPos - ($Width/2);
            $Height = $Positions[3] - $Positions[7];
            $YOffset = $YPos - 4 - $v_margin_top; //设置tip距离虚线的y轴距离

            $C_TextColor =$this->AllocateColor($this->Picture,$this->Palette[$ColorID]["R"],$this->Palette[$ColorID]["G"],$this->Palette[$ColorID]["B"]);
            imagettftext($this->Picture, $fontSize, 0, $XOffset,$YOffset,$C_TextColor,$this->FontName,$Value);
            if ($foreach_i == $d_count){
            		if ($Value >= 100){
            			$XOffset +=10;
            		}
            		imagettftext($this->Picture,$fontSize - 4, 0, $XOffset + 14,$YOffset + 24,$C_TextColor,$this->FontName,"kg");
            }
